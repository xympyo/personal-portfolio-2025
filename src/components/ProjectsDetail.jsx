  ...
  return (
    <div className="mx-5 mt-4 h-dvh">
      <div className="flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full max-w-7xl overflow-y-hidden">
          {projectData.map(
            (
              project,
              ...
              <motion.div
                ...
                transition={{
                  duration: 0.5, // How long each card's animation lasts
                  delay: index * 0.1, // Stagger delay: 0s, 0.1s, 0.2s, 0.3s, etc.
      ...
      {/* AnimatePresence for Modal Fade-in/Fade-out */}
      <AnimatePresence>
        {isModalOpen && selectedProject && (
          // Modal Overlay (fades in/out)
          <motion.div
            key="modalOverlay" // Unique key for AnimatePresence
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }} // Fades out when isModalOpen is false
            transition={{ duration: 0.3 }}
            className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4"
          >
            {/* Modal Content (slides and fades in/out) */}
            <motion.div
              key="modalContent" // Unique key for AnimatePresence
              initial={{ y: -50, opacity: 0 }} // Starts slightly above and invisible
              animate={{ y: 0, opacity: 1 }} // Slides down and fades in
              exit={{ y: -50, opacity: 0 }} // Slides up and fades out
              transition={{ duration: 0.3 }}
              className="bg-white rounded-lg p-6 max-w-xl w-full mx-auto relative shadow-2xl overflow-y-auto max-h-[90vh]"
            >
              <button
                onClick={closeModal}
